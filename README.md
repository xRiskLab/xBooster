...
### LightGBM Support 💡 (Production-Ready)

xbooster provides full support for LightGBM models with the same functionality as XGBoost. Here's how to use it:
...
**Key Features:**
- **Full Scorecard Support**: Complete implementation of `create_points()` and `predict_score()`
- **Base Score Normalization**: Proper handling of LightGBM's base score for balanced tree contributions
- **High Accuracy**: Scorecard Gini matches model Gini (typically within 0.0001)
