...
def extract_splits_info(features: str) -> List[Dict[str, Union[str, float]]]:
    ...
    for feature in feature_names:
        ...
        if match := regex.search(features):
            ...
            splits_info.append({"feature": feature, "sign": sign, "value": value})
    return splits_info
...
def plot_importance(
    ...
    **kwargs: Any,
    ...
    Raises:
        ValueError: If scorecard_constructor is not provided.
        NotImplementedError: If CatBoost constructor is provided.
    """
    if scorecard_constructor is None:
        raise ValueError("scorecard_constructor must be provided.")
    ...
    with plt.rc_context({"font.family": fontfamily, "font.size": fontsize}):
        _, ax = plt.subplots(dpi=dpi, **kwargs)
        importance_data.plot(kind="barh", width=0.75, ax=ax, fontsize=12, color=color, **kwargs)  # type: ignore

        ax.set_ylabel(None)
        ax.set_xlabel(
            ("NegLogLikelihood" if metric == "WOE" else metric),
            fontsize=12,
        )
    ...
    **kwargs: Any,
    ...
    if scorecard_constructor.max_depth > 1:
        ...
        scorecard_splits = scorecard_splits.merge(
            split_and_count_data[["Tree", "Node", "Feature", "WOE"]],
            on=["Tree", "Node"],
            how="left",
            suffixes=("", "_new"),
        )
        # Overwrite WOE with the new WOE values
        scorecard_splits["WOE"] = scorecard_splits["WOE_new"].fillna(scorecard_splits["WOE"])
        scorecard_splits.drop(columns=["WOE_new"], inplace=True)

        # Overwrite the feature value with a new one
        scorecard_splits["Feature"] = scorecard_splits["Feature_new"].fillna(
            scorecard_splits["Feature"]
        )
        scorecard_splits.drop(columns=["Feature_new"], inplace=True)
...
class TreeVisualizer:
    ...
    def parse_xgb_output(
        ...
        for line in self.tree_dump.split("\n"):
            ...
            if ":" in line:
                ...
                if "leaf" in info:
                    ...
                    nodes[node_id] = {
                        "name": self._format_node_name(leaf_value, node_metrics),
                        "depth": depth,
                    }
                else:
        ...
        def _build_tree(node_id: str, current_depth: int) -> Dict[str, Any]:
            ...
            if "children" in node:
                child_dict = {}
                for branch, target in node["children"].items():
                    if isinstance(target, str):
                        child_dict[branch] = _build_tree(target, current_depth + 1)
                    else:
                        child_dict[branch] = target
                node["children"] = child_dict
            return node

        def align_format(node: Dict[str, Any]) -> Dict[str, Any]:
            """
            ...
            if "children" in node:
                ...
                for branch, child_node in node["children"].items():
                    if isinstance(child_node, str):
                        ...
                        aligned_children[branch] = align_format(child_node)
    ...
    def _get_node_metrics(self, node_id: str) -> Optional[Dict[str, float]]:
        """
        ...
        return {
            metric: self.scorecard_frame.query(f"Node == {node_id}")[metric].item()
    ...
    def _format_node_name(
        ...
        renamed_metrics = {
            rename_dict.get(metric, metric): value for metric, value in node_metrics.items()
        }
        metrics_str = "\n".join(
            (
    ...
    def _draw_tree(
        ...
        if children := node.get("children", {}):
            ...
            first_child_pos_x = pos_x - sibling_distance * (children_count - 1) / 2

            for i, (_, child) in enumerate(children.items()):
                child_pos_x = first_child_pos_x + i * sibling_distance
                child_pos = (
                    child_pos_x,
                    -depth * level_distance - level_distance,
                )
    ...
    def plot_tree(
        ...
        **kwargs: Any,
        ...
        if self.tree_dump is None:
            self.parse_xgb_output(scorecard_constructor, num_trees)
...
def plot_catboost_importance(
    ...
    **kwargs: Any,
    ...
    """
    raise NotImplementedError(
        "CatBoost feature importance plotting is not implemented yet. "
        "Please use the constructor's built-in plotting method: constructor.plot_feature_importance()"
    )
